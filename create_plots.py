...
def plot(title, save_name, show_legend=True):
    ...
    if os.environ.get(SHOULD_SAVE_PLOTS_KEY, "True") == "True":
...
def read_datasets(data_paths):
    ...
    for path in data_paths:
        ...
        map_files.append(params_content["map_file"])        

        w = pd.read_csv(metrics)
        dataset = pd.read_csv(metrics, converters={
            ...
            "collisions_locate": Utils.string_to_float_list,
            ...
            "human_position": Utils.speical_trans,
            "global_plan": Utils.speical_trans2,
    ...
    assert len(set(map_files)) == 1, "Map files are not the same"

    return pd.concat(datasets), scenarios[0], map_files[0]
...
class PathVisualizer:
    ...
    def __init__(self, dataset ,scenario, map_file):
        self.scenario_file, self.scenario_content = PathVisualizer.read_scenario_file(scenario)
        self.map_path, self.map_content = PathVisualizer.read_map_file(map_file)
        self.human_position = dataset["human_position"].to_numpy()[0]
        self.global_plan = dataset["global_plan"].to_numpy()[0]
    ...
    def create_map_plot(self):
        ...
        # ax.imshow(self.map_img.transpose(1, 0, 2), cmap="gray")
    ...
    def create_episode_plots_for_namespaces(self, dataset, title, save_name, episode=0, differentiate="local_planner", desired_results=[], should_add_obstacles=False, should_add_collisions=False):
        ...
        for namespace in robots_tested:
            ...
            for i in iterator:
                ...
                ax.plot(path[0][:-1], path[1][:-1], label=namespace)

        self.add_obstacles_to_plot(ax,should_add_obstacles)
        self.add_collision_to_plot(ax, dataset,should_add_collisions)
        self.add_start_and_goal_to_plot(ax, paths_for_namespace["start"][0], paths_for_namespace["goal"][0])
        self.add_global_plan_to_plot(ax,should_add_obstacles)
        plot(title, save_name)
    ...
    def add_collision_to_plot(self,ax, dataset, should_add_collisions=False):
        if not should_add_collisions:
            return

        for list_ in dataset["collisions_locate"]:
            list_ = np.array(list_).reshape(-1,3)
            for local in list_:
                verts = np.array(self.ros_to_real_coord(local))
                x, y = zip(verts)
                ax.plot(x, y, "o", lw=1, color='red', ms=5, alpha=0.4)


    def add_global_plan_to_plot(self,ax,should_add_obstacles=False):
        if not should_add_obstacles:
            return

        global_plan = self.global_plan
        global_plan = global_plan.reshape(int(len(global_plan)/2),2)
        global_plan = global_plan.tolist()
        global_plan = list(set(map(tuple,global_plan)))
        global_plan = np.array(global_plan)
        verts = [self.ros_to_real_coord(pos) for pos in global_plan]
    
        x, y = zip(*verts)

        ax.plot(x, y, "o", lw=1, color="red", ms=0.1, alpha=0.4)
    ...
    def add_obstacles_to_plot(self, ax, should_add_obstacles=False):
        ...
        obstacles = self.scenario_content["peds_num"]
        human_position = self.human_position.reshape(int(len(self.human_position)/(2*obstacles)),2*obstacles)
        for i in human_position:
            pos = i.reshape(obstacles,2)
            verts = [self.ros_to_real_coord(waypoint) for waypoint in pos]
        
            x, y = zip(*verts)

            ax.plot(x, y, "x", lw=1, color="black", ms=1, alpha=0.4)
    ...
    def read_scenario_file(scenario):
        name = os.path.join(rospkg.RosPack().get_path("simulation-setup"), "scenario_config", scenario)
    ...
    def read_map_file(map_name):
        map_path = os.path.join(rospkg.RosPack().get_path("simulation-setup"), "maps", map_name)
...
def create_plots_from_declaration_file(declaration_file):
    ...
    ## Dataset setup
    dataset, scenario, map_file = read_datasets(declaration_file["datasets"])
    ## Plot Result
    ...
    # Plot paths
    path_visualizer = PathVisualizer(dataset ,scenario , map_file)
...
if __name__ == "__main__":
    ...
    with open(rospkg.RosPack().get_path("evaluation")+"/plot_declarations/"+ args.declaration_file) as file:
