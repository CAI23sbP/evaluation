...
class Metrics:
    def __init__(self,name):
        self.name=name
        self.dir = rospkg.RosPack().get_path("evaluation")+f"/data/{self.name}/"
        print(self.dir)
        ...
        human_pose = pd.read_csv(self.dir + "/human.csv", converters={
            "data": Utils.string_to_float_list,
        })
        global_plan = pd.read_csv(self.dir + "/global_plan.csv", converters={
            "data": Utils.string_to_float_list,
        })
        ...
        human_pose = human_pose.rename(columns={"data": "human_position"})
        global_plan = global_plan.rename(columns={"data": "global_plan"})
        
        data = pd.concat([episode, laserscan, odom, cmd_vel, start_goal, human_pose , global_plan], axis=1, join="inner")
        ...
        data.to_csv(os.path.join(self.dir, "metrics.csv"))

    def analyze_episode(self, episode, index):
        global_plan, human_position ,positions, velocities = [], [], [], []
        ...
        collisions, collision_amount, collisions_locate = self.get_collisions(
            episode["laserscan"],
        ...
        for pose in episode["human_position"]:
            human_position.append(pose)
            
        for pose in episode["global_plan"]:
        ...
        return {
            ...
            "collisions_locate": collisions_locate,
    ...
    def get_mean_human_pose(self,episode):
        print(episode["human_position"])
    ...
    def get_collisions(self, laser_scans, lower_bound, positions):
        """
        ...
        for i, scan in enumerate(laser_scans):
            ...
            if is_collision:
                collisions.append(i)
                collisions_locate.append([positions[i][0],positions[i][1],positions[i][2]])
        ...
        return collisions, collision_amount,collisions_locate
    ...
    def get_robot_params(dir):
        with open(dir+ "params.yaml") as file:
